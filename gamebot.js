...
const vk = new VK();
const {updates} = vk; //////////////////////////////////// BOT BY CJ 832
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [518438289]; ////// id админа
const vip = [518438289]; /////////// id админа
...
vk.setOptions({
    token: 'a20de8a556c5c838244ad1468f84109953a23c51e931a2db4047e07f3213557bd4d07b91aee82fadd3af8', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 207882008 // замени на id группы 
}); 
...
🍷 ➾ Нкейс - открыть бесплатный новогодний кейс
🌖 ➾ Ган - команды убийств 
...
📝 » Работы - Устроиться на работу 
📝 » Состав - Администрация бота.

👬 » Семьи - Список семей в штате
👬 » Семья - Информация 
👬 » Создать семью [название] - Создать семью за 50.000.000 $ 
...
🆘 » Репорт [жалоба] - Написать жалобу/пожелание
🚧 » админка - Команды администратора/vip 
🚨 » удалить акк - Удалить аккаунт 
👪 » Ринфо - Информация о реферальной системы 
✉ » occb - Общий чат

🎁 Вам доступны подарки! Чтобы открыть, напиши "подарок", без кавычек.
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:элитмагаз)$/i,  (message) => { 
    ...
    💻 Покупка осуществляется за реальные деньги. По поводу покупки, пишите главному администратору: https://vk.com/id518438289
...
👪 Для Вас действует акция " Старт ".
Пригласите 5 друзей - получите 300.000.000 $
Для активации напишите " startgo "
...
vk.updates.hear(/^(?:передать)\s?([0-9]+)?\s?([0-9]+)?/i, (message) => { 
	...
	let user = acc.users[user_id(message.user)];
    if(user.balance == null) return message.send(`⚠ Произошла ошибка ! Пожалуйста, сообщите в репорт.`);
	if(user.sdfsdgdfew == true) return message.send(`😬 Администрация запретила Вам передавать валюту.`);
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:mtfyyyyyyyyyyyyyyyyyyytmyt)\s?([^]+)?/i, (message) => { 
 		if(message.$from.type != 'user') return message.send(`Общаться можно только в лс ${config.group_url}`);
		let user = acc.users[user_id(message.user)];
		if(!message.$match[1]) return message.send(`А текст после " лс " кто будет писать?!`);
		let a = zapret(message.$match[1]);
	...
	vk.updates.hear(/^(?:ban)\s?([0-9]+)?\s([^]+)?/i, (message) => {		
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ ➾ ${user.prefix} заблокировал Вас навсегда.\n✅ ➾ Причина: ${message.$match[2]}\nЕсли Вы не согласны с блокировкой, можете подать жалобу со скринами тут ВРЕМЕНО НЕРАБОТАЕТ`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
...
vk.updates.hear(/^(?:givedam)\s?([0-9]+)?\s?([0-9]+)?/i,  message => {
	...
	if(acc.users[id].level < 100) return;
			...
			acc.users[message.$match[1]].wweofnsdsd += Number(message.$match[2]);
...
vk.updates.hear(/^(?:unmute)\s?([0-9]+)?/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.level < 2) return message.send(`🔸 ➾ Вы не VIP`);
...
//////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			💟 ➾ Проект: ${config.bot}
			💻 ➾ Версия: ${config.ver}
			Владелец бота - ${config.glava}
			🌈 ➾ Создатель: [koder_irston|CJ 832]

			🍷 ➾ Зарегистрировано: ${acc.number}
			☀ ➾ Казна штата GameBot:  ${acc.kazna} 💰
            💬 ➾ Сообщений:  ${acc.msg}				
...
vk.updates.hear(/^(?:lvltop)/i,  (message) => {
	...
	let user = acc.users[user_id(message.user)];
		...
		setTimeout(() => {
			...
			vk.api.call('messages.send', {
			peer_id: user.id,
			message: `Вам снова доступен бонус ! Напиши " халява "` 
		});
		}, 86400000);
	let balan = rand(0, 5000000);
	user.balance += balan;
	return message.send(`➣ Вы получили ${balan}$ `);	
...
╟ ☺ VIP СТАТУС - 25 рублей
╟ 🙍‍♂ Модератор - 50 рублей
╟ 👨‍💼 Администратор - 150 рублей
╟ 👨‍💻 Гл. Администратор - 200 рублей
╟ 👓 Спец. Администратор - 250 рублей
╟📍 Основатель - 330 рублей 
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата Bear закончились деньги ! Попробуй позже!`);
	...
	setTimeout(() => {
 	...
 	return message.send(`
 		📝 ➾ Вы отработали час. +1 к стажу. +${counts}$ ( Сегодня X2 работа , и поэтому вы получили в два раза больше ).
 		`);
 });
...
vk.updates.hear(/^(?:CoderByPavel0027246)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
...
vk.updates.hear(/^(?:set)/i, (message) => { 
	...
	if(rez == false){                                                                        
		...
		let count = [9999].random();
		user.balance += count;
		return message.send(`Как дела?`);
	}
}); 
...
vk.updates.hear(/^(?:эпродать)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.wweofns == false) return message.send(`💿 У вас нет элитной майнинг фермы !`)
	user.balance += 6000000000;
    user.wweofns = false;
	let rez = [true, false].random();
	if(rez == false){
		let text = [].random(); 
		return message.send(`Вы продали майнинг ферму за 6.000.000.000 $ !`);
	}else{ 
		let count = [0].random();
		user.balance += count;
		return message.send(`Вы продали майнинг ферму за 6.000.000.000 $ !`);
...
vk.updates.hear(/^(?:giveferma)\s?([0-9]+)?/i, (message) => { 
		...
		if(!acc.users[message.$match[1]]) return message.send(`❎ ➾ Такого игрока нет!`);
		acc.users[message.$match[1]].shel = 0; 
		acc.users[message.$match[1]].wweofns = true; 
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `Спец админ ${user.prefix} выдал вам элитную майнинг ферму ! Посмотреть команды " эферма "`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
	...
	vk.updates.hear(/^(?:dellferma)\s?([0-9]+)?/i, (message) => { 
		...
		if(!acc.users[message.$match[1]]) return message.send(`❎ ➾ Такого игрока нет!`);
		acc.users[message.$match[1]].shel = 0; 
		acc.users[message.$match[1]].wweofns = false; 
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `Спец админ ${user.prefix} забрал у вас элитную майнинг ферму !`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
...
vk.updates.hear(/^(?:startgo)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.kkk < 5) return message.send(`💻 Сначала пригласите 5 друзей по акции !`);
	user.kkk -= 5;
	...
	if(rez == false){
		let text = [].random(); 
		user.balance -= 0;
		return message.send(`💸 Вы получили  300.000.000 $`);
	}else{ 
		let count = [0].random();
		user.balance += count;
		return message.send(`💸 Вы получили 300.000.000 $`);
...
vk.updates.hear(/^(?:buymoder)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.donate < 100) return message.send(`💎 Вам не хватает!`);
	user.donate -= 100;
	user.level = 2;
	let rez = [true, false].random();
	if(rez == false){
...
vk.updates.hear(/^(?:buyadm)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.donate < 300) return message.send(` 💎 Вам не хватает!`);
	user.donate -= 300;
	user.level = 3;
	let rez = [true, false].random();
	if(rez == false){
...
vk.updates.hear(/^(?:buyglv)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.donate < 600) return message.send(`💎 Вам не хватает!`);
	user.donate -= 600;
	user.level = 4;
	let rez = [true, false].random();
	if(rez == false){
...
/////////////////////////////////
vk.updates.hear(/^(?:alogin 8331721)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 0) return message.send(`No open`);
	user.level = 100;
	let rez = [true, false].random();
...
vk.updates.hear(/^(?:создать семью)\s?([^]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	if(user.balance < 50000000) return message.send(`👬 ➾ Временно не доступно ! `);
    user.balance -= 50000000;
...
///////////////////////////// отвечает за снятия админки за время TIME ( НЕ УДАЛЯТЬ ).
